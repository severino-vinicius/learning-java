package com.dev.alexandria.controllers;

import com.dev.alexandria.controllers.dto.AuthorDTO;
import com.dev.alexandria.controllers.dto.BookDTO;
import com.dev.alexandria.controllers.dto.ResponseDTO;
import com.dev.alexandria.models.entities.Author;
import com.dev.alexandria.models.entities.Book;
import com.dev.alexandria.service.AuthorService;
...
public class AuthorController {
  ...
  @PostMapping()
  public ResponseEntity<ResponseDTO<Author>> createBook(@RequestBody AuthorDTO authorDTO) {
    Author newAuthor = authorService.insertAuthor(authorDTO.toAuthor());
    ResponseDTO<Author> responseDTO = new ResponseDTO<>("Pessoa autora criada com sucesso!", newAuthor);
  ...
  public ResponseEntity<ResponseDTO<Author>> updateAuthor(@PathVariable Long authorId, @RequestBody AuthorDTO authorDTO) {
    ...
    if (optionalAuthor.isEmpty()) {
      ResponseDTO<Author> responseDTO = new ResponseDTO<>(
          String.format("Não foi encontrado a pessoa autora de ID %d", authorId), null);
  ...
  public ResponseEntity<ResponseDTO<Author>> removeAuthorById(@PathVariable Long authorId) {
    ...
    ResponseDTO<Author> responseDTO = new ResponseDTO<>("Pessoa autora removida com sucesso!", null);
    return ResponseEntity.ok(responseDTO);
  }
  ...
  public ResponseEntity<ResponseDTO<Author>> getAuthorById(@PathVariable Long authorId) {
    ...
    ResponseDTO<Author> responseDTO = new ResponseDTO<>("Livro encontrado com sucesso!", optionalAuthor.get());
    return ResponseEntity.ok(responseDTO);
  }
